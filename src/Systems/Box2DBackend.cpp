...
void Box2DBackend::Init(entt::registry& registry) {
    ...
    // PRE-STEP
    {
        ...
        for (auto entity : view) {
            ...
            if (!b2Body_IsValid(GetB2Body(rb.RuntimeBody))) {
                ...
                rb.RuntimeBody = ToHandle(b2CreateBody(m_worldId, &bodyDef));
                ...
                if (!b2Joint_IsValid(GetB2Joint(joint.RuntimeJoint)) && registry.valid(joint.TargetEntity)) {
                    if (registry.all_of<RigidBodyComponent>(joint.TargetEntity)) {
                        ...
                        if (b2Body_IsValid(GetB2Body(targetRb.RuntimeBody))) {
                            ...
                            joint.RuntimeJoint = ToHandle(b2CreateDistanceJoint(m_worldId, &jointDef));
                        }
                    }
                }
            }
...
void Box2DBackend::Step(entt::registry& registry, float dt) {
    ...
    // POST-STEP
    {
        ...
        auto checkJointBreak = [this, &registry](auto entity, auto& joint) {
...
void Box2DBackend::SetMaterial(entt::registry& registry, entt::entity entity,
...
float Box2DBackend::GetJointForce(entt::registry& registry, entt::entity entity) {
    if (registry.all_of<DistanceJointComponent>(entity)) {
        auto& joint = registry.get<DistanceJointComponent>(entity);
...
void Box2DBackend::DestroyJoint(entt::registry& registry, entt::entity entity) {
    if (registry.all_of<DistanceJointComponent>(entity)) {
        auto& joint = registry.get<DistanceJointComponent>(entity);
        if (b2Joint_IsValid(GetB2Joint(joint.RuntimeJoint))) {
            b2DestroyJoint(GetB2Joint(joint.RuntimeJoint));
