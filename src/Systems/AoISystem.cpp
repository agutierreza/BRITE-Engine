...
void AoISystem::Update(entt::registry& registry, const AoIConfig& config) {
    ...
    for (auto entity : view) {
        ...
        if (!aoi.Active) continue;

        float dx = trans.Position.x - config.Center.x;
        if (config.WorldWidth > 0.0f) {
            if (dx > config.WorldWidth * 0.5f) dx -= config.WorldWidth;
            else if (dx < -config.WorldWidth * 0.5f) dx += config.WorldWidth;
        }

        float dy = trans.Position.y - config.Center.y;
        if (config.WorldHeight > 0.0f) {
            if (dy > config.WorldHeight * 0.5f) dy -= config.WorldHeight;
            else if (dy < -config.WorldHeight * 0.5f) dy += config.WorldHeight;
        }
