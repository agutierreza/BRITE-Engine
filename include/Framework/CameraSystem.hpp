...
// Helper to calculate the camera zoom required to fit a physical view height
// (in meters) onto the internal screen height.
...
inline void UpdateCamera(Camera2D& camera, CameraFollowComponent& follow, Vector2 targetPos, Vector2 targetVelocity,
                         float dt) {
    switch (follow.mode) {
    ...
    case CameraMode::LeadVelocity:
        ...
        if (Vector2Length(targetVelocity) < 0.1f)
            desiredLead = {0.0f, 0.0f}; // Reset if not moving
